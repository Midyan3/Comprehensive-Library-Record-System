...
Manual::Manual(std::string name, std::string author, int page_count, std::string model, bool is_digital, std::string url, bool aid) : Book(name, author, page_count, is_digital), has_visual_aid_{aid}, device_{model}
{
  ...
  else if(url == "https://www.NONE" || url == "NONE"){
    website_ = "NONE";
    has_website_ = false;
  }
  else {
...
/*
  @param  : a string representing the device
  @return : a string representing the device without the version number
*/
std::string Manual::helper(std::string device){
  for(int i = 0; i < device.length(); i++){
    if(device[i] == '-'){
      return device.substr(0, i);
    }
  }
  return device;
}
/*
  @post   : displays the title, author, device, website, page count, 
            and digital availability of the manual
*/
void Manual::display(){
   std::cout<<getTitle()<<" is written by "<<getAuthor()<<" company for device: "<<helper(getDevice())<<". Website: "<<getWebsite()<<". Page Count: "<<getPageCount()<<". "<<(isDigital() ? "It is": "It is not")<< " available digitally."<<"\n"; 
}
/*
  @param  : a string representing the key
  @post   : displays the title, author, device, website, page count, 
            and digital availability of the manual
*/

void Manual::displayFilter(std::string key){
  if(key == helper(getDevice())){
    display(); 
  }
}
